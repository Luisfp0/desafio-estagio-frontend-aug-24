...
export function formatWhatsAppDate(dateString: string): string {
  ...
  const localDateWithoutTime = new Date(localDate.toDateString());
  const localNowWithoutTime = new Date(localNow.toDateString());

  const diffInMs = localNowWithoutTime.getTime() - localDateWithoutTime.getTime();
  ...
  if (diffInDays === 0) {
    ...
    return localDate.toLocaleDateString('pt-BR', { day: 'numeric', month: 'numeric' });
  } else {
    return localDate.toLocaleDateString('pt-BR', { day: 'numeric', month: 'numeric', year: 'numeric' });
  }
}